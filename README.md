...
Process figure transitions.

![](https://p4.ssl.qhimg.com/t016ccc24015fab0251.gif)
...
const {Figure2D, Transition, Mesh2D, Renderer} = meshjs;

const transitions = [];
const f0 = new Figure2D();
f0.addPath(paths[0]);
...
for(let i = 0; i < paths.length - 1; i++) {
  ...
  f2.addPath(paths[i + 1]);
  transitions.push(new Transition(f1, f2));
  f1 = f2;
}

transitions.push(new Transition(f1, f0));
...
const T = 1000;
let startTime;
const mesh = new Mesh2D(f0, canvas);
...
function update() {
  ...
  p = Math.min(p, 1.0);
  mesh.contours = transitions[idx % transitions.length].progress(p).contours;
  renderer.clear();
  renderer.drawMeshes([mesh]);
  if(p >= 1.0) {
    idx++;
